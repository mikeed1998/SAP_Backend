...
<body style="background-color: #201E1F;">
    ...
    <div class="col-11 mx-auto">
        <div class="row mb-4 px-2">
            <a href="{{ route('config.blog.index') }}" class="col mt-5 mb-5 col-md-2 btn btn-sm grey darken-2 text-dark bg-white mr-auto"><i class="fa fa-reply"></i> Regresar</a>
        </div>
    </div>
        ...
        <div class="row">
            <div class="col-11 mb-5 mx-auto">
                <form id="miFormulario" method="post" action="{{ route('config.blog.update', ['id' => $post->id]) }}" enctype="multipart/form-data">
                    ...
                    <input type="text" name="titulo" class="form-control py-2 mt-1 mb-3 text-white" id="titulo" style="background-color: #201E1F;" placeholder="Titulo" value="{{ $post->titulo }}">
                    <label for="descripcion" class="form-label h4 text-white">Descripción de la entrada</label>
                    <textarea name="descripcion" id="descripcion" cols="30" rows="6" maxlength="1000" class="form-control text-white" style="background-color: #201E1F;" placeholder="Esta descripción se mostrará en la miniatura de este blog en la sección donde aparecen todos los blogs"> {{ $post->resumen }} </textarea>
                    <p id="contador-caracteres">Caracteres restantes: <span id="caracteres-restantes"></span></p>
                    <label for="portada" class="form-label h4 text-white mt-1">Portada de este post</label>
                    <input type="file" id="portada" name="portada" class="form-control" style="background-color: #201E1F;" value="{{ $post->portada }}">
                    ...
                    <input type="color" id="colorPicker" name="colorPicker" value="{{ $post->color }}" onchange="actualizarColor()">
                    <hr class="mb-3">
                    <label for="summernote" class="form-label h4 text-white mt-1">Contenido del post</label>
                    <textarea id="summernote" name="contenido">{{ $post->post }}</textarea>
                    <button class="btn-lg btn-success mt-2 w-100" type="submit">Actualizar post</button>
                </form>
            </div>
        </div>
    </div>
